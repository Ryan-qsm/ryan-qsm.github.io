<div class="home-hero">
  <h1>钱实明的个人网站</h1>
  <p>记录学习笔记与生活分享</p>
</div>

<div class="grid cards" markdown>

-   :material-airplane:{ .lg .middle } **旅行**

    ---

    在路上，看世界

    [:octicons-arrow-right-24: 进入](travel/index.md)

-   :material-book-open-variant:{ .lg .middle } **文学电影**

    ---

    书与光影的对话

    [:octicons-arrow-right-24: 进入](literature/index.md)

-   :material-food:{ .lg .middle } **美食**

    ---

    味觉记忆

    [:octicons-arrow-right-24: 进入](food/index.md)

-   :material-school:{ .lg .middle } **学习记录**

    ---

    持续成长

    [:octicons-arrow-right-24: 进入](study/index.md)

-   :material-run:{ .lg .middle } **体育**

    ---

    运动与活力

    [:octicons-arrow-right-24: 进入](sports/index.md)

</div>
